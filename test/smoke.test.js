...
// 16c) 選択肢が図の問題: choiceImages（肢別画像）／choicesImage（合成1枚）のUI表示
{
  // 肢別画像（choiceImages）
  const percQ = api.BANK.find(q => q.choiceImages);
  assert(percQ, '肢別画像問題(choiceImages)が実データに存在');
  assert(['ア', 'イ', 'ウ', 'エ'].every(k => percQ.choiceImages[k] && percQ.choiceImages[k].indexOf('questions/img/') === 0), 'choiceImagesはア〜エ4肢の画像パス');
  api.resetProgress();
  api.state.practice = { mode: 'random', label: 't', queue: [percQ.id], idx: 0, picked: null };
  api.go('practice');
  assert(elements.main.innerHTML.includes('class="choice-img"') && elements.main.innerHTML.includes(percQ.choiceImages['ア']), '肢別画像が各選択肢に表示される');
  api.answerQuiz(percQ.answer);
  assert(api.state.practice.picked === percQ.answer, '肢別画像問題でも解答できる');
  api.endPractice();
  // 合成1枚（choicesImage）
  const compQ = api.BANK.find(q => q.choicesImage);
  assert(compQ, '合成画像問題(choicesImage)が実データに存在');
  api.state.practice = { mode: 'random', label: 't', queue: [compQ.id], idx: 0, picked: null };
  api.go('practice');
  assert(elements.main.innerHTML.includes('class="choices-composite"') && elements.main.innerHTML.includes(compQ.choicesImage), '合成選択肢画像が表示される');
  api.answerQuiz(compQ.answer);
  assert(api.state.practice.picked === compQ.answer, '合成画像問題でも解答できる');
  api.endPractice();
}
console.log('OK 選択肢が図の問題（肢別choiceImages・合成choicesImage）のUI表示・解答');
