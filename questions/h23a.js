...
[
 ...
 },
 {
  ...
  "answer": "エ",
  "hasImage": false,
  "text": "n個の正の整数 x_1,x_2,…,x_n が並んだ線形リストを [x_1,x_2,…,x_n] で表し，空リストは[ ]で表す。次のように再帰的に定義される関数 func(L) を，L=[1,3,2] を実引数として呼び出したとき，print文によって表示される数字はどれか。ここで，プログラム中の=は等号，:=は代入を表す。 〔関数の定義〕first([x_1,x_2,…,x_n])はx_1を返す。butfirst([x_1,x_2,…,x_n])は[x_2,…,x_n]を返す。butfirst([x])は[ ]を返す。max(x,y)は，x≧yであればxを返し，そうでなければyを返す。func(L) begin if L = [] then return 0; A := first(L); B := func(butfirst(L)); C := max(A, B); print C; return C; end",
  "choices": {
   "ア": "123",
   "イ": "133",
   "ウ": "223",
 ...
 },
 {
  ...
  "answer": "イ",
  "hasImage": true,
  "text": "価格設定年月日にNULLを含む\"商品\"表に対して，次の問合せを行った。この検索結果の行数は幾つか。 SELECT 商品コード FROM 商品 WHERE 価格設定年月日 < '2011-12-01'",
  "choices": {
   "ア": "1",
   "イ": "2",
   "ウ": "3",
 ...
 },
 {
  ...
  "answer": "エ",
  "hasImage": false,
  "text": "10Mビット／秒のLANで接続された4台のノード(A，B，C，D)のうち， 2組(AとB，CとD)のノード間でそれぞれ次のファイル転送を行った場合， LANの利用率はおよそ何%か。ここで，転送時にはファイルの大きさの30%に当たる各種制御情報が付加されるものとする。また， LANではリピータハブが使用されており，更に衝突は考えないものとする。 ファイルの大きさ：平均1,000バイト ファイルの転送頻度：平均60回／秒(1組当たり)",
  "choices": {
   "ア": "2",
   "イ": "6",
   "ウ": "10",
