...
[
 ...
 },
 {
  ...
  "answer": "イ",
  "hasImage": false,
  "text": "自然数をキーとするデータを，ハッシュ表を用いて管理する。キーxのハッシュ関数h(x)を h(x)＝x mod n とすると，キーaとbが衝突する条件はどれか。ここで，n はハッシュ表の大きさであり，x mod n は x を n で割った余りを表す。",
  "choices": {
   "ア": "a＋bがnの倍数",
   "イ": "a－bがnの倍数",
   "ウ": "nがa＋bの倍数",
 ...
 },
 {
  ...
  "answer": "ア",
  "hasImage": true,
  "text": "文字列を引数とする関数 len，first，butfirst を用いて，関数 comp を再帰的に定義した。 comp(\"11\"，\"101\")を呼び出したとき，返されるものはどれか。 〔関数の定義〕len(S)：文字列Sの長さを返す。Sが空文字列のときは0を返す。first(S)：文字列Sの先頭の1文字のASCIIコードを返す。Sが空文字列のときはエラーを返す。butfirst(S)：文字列Sの先頭の1文字を除いた残りの文字列を返す。 Sが空文字列のときはエラーを返す。",
  "choices": {
   "ア": "-1",
   "イ": "0",
   "ウ": "1",
 ...
 },
 {
  ...
  "answer": "イ",
  "hasImage": true,
  "text": "IPネットワークにおいて，二つのLANセグメントを，ルータを経由して接続する。ルータの各ポート及び各端末のIPアドレスを図のとおりに設定し，サブネットマスクを全ネットワーク共通で255.255.255.128とする。 ルータの各ポートのアドレス設定を正しいとした場合，IPアドレスの設定を正しく行っている端末の組合せはどれか。",
  "choices": {
   "ア": "AとB",
   "イ": "AとD",
   "ウ": "BとC",
