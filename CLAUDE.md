...
- **収録状況（manifest.js）— 主要9回は完了・配信確認済み（合計675問）**:
  ...
  - **進捗（2026-07-20時点）**: 32回中**28回が解説完了**（hasExpl:true）。r2a〜h29h＋h28a〜h24h＋**h23a・h23h完了**。**残り4回: h22a→h22h→h21a→h21h**。コーディネーター指示により**1回ごとにverify→smoke→commit→push**。injectスクリプトは `scratchpad/inject_expl.py`（in-place置換・空欄のみ・既存保護）。図表問はfitzで4倍拡大して数値確認する運用が定着。注入後は必ず「まだ空のexplanationが残っていないか」を確認（h26hでq4を書き漏らした実例あり）。過去回と同一問題の解説は、text/choices完全一致を機械確認してから流用（数値違いの類題に注意）。
