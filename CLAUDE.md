...
- **収録状況（manifest.js）— 主要9回は完了・配信確認済み（合計675問）**:
  ...
  - **次にやること**: **フェーズ2＝全年度への拡張**（道場収録の平成21春〜、約41回3,280問）。先に各回のIPA解答例PDFから正解抽出→`data/answers.json`へ追加・検証→`build_from_dojo.py`で問題文・選択肢・図表を抽出→収録。正解の裏取り不可の回（ベクター化PDF等）は収録見送り・報告。新規回は解説なし先行でよく、後続で同じ手法で解説を足す。
