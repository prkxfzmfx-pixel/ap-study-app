...
- **収録状況（manifest.js）— 主要9回は完了・配信確認済み（合計675問）**:
  ...
  - **進捗（2026-07-20時点）＝解説生成 全32回完了**: 全32回・2,384問すべてに解説を付与済み（全回 `hasExpl:true`、空explanation 0、verify OK・smoke全PASS・push・配信確認済み）。最後にh22a→h22h→h21a→h21h（h21hが最終回、v19で配信）を完了。injectスクリプトは `scratchpad/inject_expl.py`（in-place置換・空欄のみ・既存保護）。図表問はfitzで4倍拡大して数値確認する運用が定着。注入後は必ず「まだ空のexplanationが残っていないか」を確認（h26hでq4・h21hでq77を書き漏らした実例あり）。過去回と同一問題の解説は、text/choices完全一致を機械確認してから流用（数値違いの類題に注意）。**次回タスク: 解説の品質見直し・平成回のcat（分野）分類の付与検討など。新規解説生成は残っていない。**
  - **表示品質修正（v16〜v18でリリース済み）**: 問題文・選択肢の上付き/下付きは `[[sup:x]]`/`[[sub:x]]`、コードブロック（SQL・擬似言語）は `[[code:...]]`、**下線（主キー/外部キー表記）は `[[u:x]]`** プレースホルダで保持し、`qTextHtml()` が `<sup>/<sub>/<pre class=qcode>/<u>` に描画。パーサ（build_from_dojo.py の strip_tags）が対応markup（`<sup>/<sub>/<div class=code>/<u>` 等）を各プレースホルダで出力。旧データは `regen_text.py` で全回再生成済み。r2a問1のlog底・r2a問28(正規化)の主キー下線が復元済み。v17で演習タブに習熟ドーナツ・年度別タブ見出し整理。
