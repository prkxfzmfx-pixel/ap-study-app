...
- **収録状況（manifest.js）— 主要9回は完了・配信確認済み（合計675問）**:
  ...
  - **解説生成の残り（新規22回・約1,500問）**: 令和7秋〜令和3春と同じ品質仕様（正解肢2〜3文＋誤答肢ア〜エ各1文・answers.jsonと矛盾禁止）で、新しい回から順（r2a→r1a→h31h→…→h21h）に生成。手法はフェーズ1と同じ（解説マップJSON→`scratchpad/inject_expl.py <key> <map.json>`で空欄のみ注入→manifestと.js metaの`hasExpl`をtrueへ）。図表問は該当図を目視。**効率化: push・配信確認は3〜4回分ごとに1回にまとめる**（コミットは回ごと可）。verifyとsmokeは各push前に必ず実行。
