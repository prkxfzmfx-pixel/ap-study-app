...
- **収録状況（manifest.js）— 主要9回は完了・配信確認済み（合計675問）**:
  ...
  - **進捗（2026-07-19時点）**: r2a・r1a・h31h・h30a・h30h・h29a・h29h・**h28a・h28h＝解説完了**（hasExpl:true）。**残り14回: h27a→h27h→h26a→h26h→h25a→h25h→h24a→h24h→h23a→h23h→h22a→h22h→h21a→h21h**。コーディネーター指示により**1回ごとにverify→smoke→commit→push**（従来の3〜4回まとめ運用から変更）。injectスクリプトは `scratchpad/inject_expl.py`（in-place置換・空欄のみ・既存保護）。
